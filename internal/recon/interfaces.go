// Package recon provides reconnaissance orchestration capabilities for the debug agent.
//
// This package coordinates the execution of network reconnaissance tools through multiple phases
// (discover, probe, scan, domain) and extracts structured entities from tool outputs into the
...
//
// The primary use case is automated local network reconnaissance where the agent systematically
// discovers hosts, probes services, scans for vulnerabilities, and enumerates domains to build
// a comprehensive knowledge graph of the network environment.
package recon
...
const (
	...
	// PhaseScan performs vulnerability scanning against discovered endpoints.
	// Tools: nuclei
	// Entities: finding nodes with AFFECTS relationships to hosts/endpoints
...
// technologies, findings, and domain information.
type ReconRunner interface {
	...
	// probe uses httpx, scan uses nuclei, domain uses subfinder/amass).
	//
	...
	// - scan: URLs (e.g., ["http://192.168.1.10:8080"])
	...
	// ports identified, HTTP endpoints probed, findings detected, and overall execution duration.
	//
...
type PhaseResult struct {
	...
	// This includes all node types (host, port, endpoint, technology, finding, domain, subdomain).
	NodesCreated int

	// RelationsCreated is the count of knowledge graph relationships created from this phase's
	// tool outputs. This includes relationships like HAS_PORT, EXPOSES, USES_TECHNOLOGY, AFFECTS.
...
type ReconResult struct {
	...
	// TotalFindings is the count of unique finding nodes (vulnerabilities, misconfigurations)
	// discovered across all phases. Derived primarily from PhaseScan tool outputs (nuclei).
	TotalFindings int
