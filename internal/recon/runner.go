...
func (r *DefaultReconRunner) RunPhase(ctx context.Context, phase Phase, targets []string) (*PhaseResult, error) {
	...
	case PhaseScan:
		r.runScanPhase(ctx, targets, result)
	case PhaseDomain:
...
// runScanPhase executes the scan phase using nuclei.
func (r *DefaultReconRunner) runScanPhase(ctx context.Context, targets []string, result *PhaseResult) {
	logger := r.harness.Logger()

	if len(targets) == 0 {
		logger.WarnContext(ctx, "no targets for scan phase")
		return
	}

	logger.InfoContext(ctx, "running nuclei scan", "target_count", len(targets))

	// nuclei accepts a list of URLs
	input := map[string]any{
		"targets": targets,
	...
	output, err := r.harness.CallTool(ctx, "nuclei", input)
	if err != nil {
		logger.ErrorContext(ctx, "nuclei failed", "error", err)
		result.Errors = append(result.Errors, fmt.Errorf("nuclei failed: %w", err))
		return
	}

	result.ToolsRun = append(result.ToolsRun, "nuclei")

	// Extract entities from nuclei output using taxonomy extractor
	if output != nil {
		outputJSON, err := json.Marshal(output)
		if err != nil {
			logger.ErrorContext(ctx, "failed to marshal tool output", "error", err)
		...
		nodes, rels, err := r.extractor.Extract(ctx, "nuclei", outputJSON)
		if err != nil {
			logger.ErrorContext(ctx, "failed to extract entities", "tool", "nuclei", "error", err)
			result.Errors = append(result.Errors, fmt.Errorf("extraction failed: %w", err))
		} else {
			result.NodesCreated += nodes
			result.RelationsCreated += rels
...
// RunAll executes all reconnaissance phases in sequence: discover -> probe -> scan -> domain.
...
func (r *DefaultReconRunner) RunAll(ctx context.Context, subnet string, domains []string) (*ReconResult, error) {
	...
	if len(probeTargets) > 0 {
		...
		// Extract HTTP URLs for vulnerability scanning
		scanTargets, err := r.extractScanTargets(ctx)
		if err != nil {
			logger.ErrorContext(ctx, "failed to extract scan targets", "error", err)
			scanTargets = []string{}
		}

		// Phase 3: Vulnerability scanning
		if len(scanTargets) > 0 {
			scanResult, err := r.RunPhase(ctx, PhaseScan, scanTargets)
			if err != nil {
				return nil, fmt.Errorf("scan phase failed: %w", err)
			}
			result.Phases = append(result.Phases, scanResult)
		}
	}
	...
	for _, phaseResult := range result.Phases {
		...
		result.TotalFindings += countFindings(phaseResult)
	}
...
// extractScanTargets queries the knowledge graph to find HTTP endpoints.
// Returns a list of URLs to scan with nuclei.
func (r *DefaultReconRunner) extractScanTargets(ctx context.Context) ([]string, error) {
	// Query for endpoint nodes
	// This is a simplified implementation - a real implementation would query Neo4j
	// For now, return empty slice as we'll implement GraphRAG queries later
	return []string{}, nil
}
...
func countFindings(result *PhaseResult) int {
	if result.Phase == PhaseScan {
		// In scan phase, all nodes are findings
